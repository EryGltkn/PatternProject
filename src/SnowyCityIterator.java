public class SnowyCityIterator implements CityIterator {
    ...
    public City[] getCityList() {
        ...
        for (int i = 0; i < cityList.length; i++) {
             if (cityList[i] != null && "Snowy".equalsIgnoreCase(cityList[i].getCurrentWeatherCondition())) {
                snowyCities[index++] = cityList[i];
            }
        }
        City[] result = new City[index];
        System.arraycopy(snowyCities, 0, result, 0, index);
        return result;
    }
}